<script>
  ...
  //Carousel
  let carousel;
  let currentIndex = 1; // Start at the middle image

  function handlePageChange(event) {
    currentIndex = event.detail; // Update currentIndex when page changes
  }

  //Visible and hover images
  let isVisible = false;
...
<section class="mb-12 bg-gray-100 py-4">
    ...
    >
      ...
      <a href="#react" class={currentIndex === 0 ? "z-50" : "z-0"}>
        <div class="relative group">
          <img
            class="w-20 h-20 rounded-xl shadow-lg carousel-item group-hover:opacity-80 cursor-pointer"
          ...
          />
          <p
            class="absolute bottom-20 left-5 w-full text-center opacity-0 group-hover:opacity-100 bg-black bg-opacity-50 text-white p-2 transition-opacity duration-300"
          >
  ...
  <div class="sm:p-6 sm:container sm:mx-auto mt-12">
    ...
    <div id="vok" class="grid grid-cols-2 gap-6 mt-60">
      <div>
        ...
        <div
          class="flex md:flex-row flex-col gap-6 justify-normal md:justify-center h-screen"
        >
          <!-- svelte-ignore a11y_no_noninteractive_tabindex -->
          <img
            class="md:h-60 h-80 md:w-1/2 w-72 transition-transform duration-300 ease-in-out translate-x-1/4 md:focus:z-50 md:translate-x-0 focus:translate-x-0 md:focus:translate-x-3/4 md:focus:scale-[2] focus:scale-[2.5] hover:scale-[2.5]"
      ...
      <div>
        <img
          class="h-3/6 sm:h-80 pl-12 justify-center mt-12 transition-transform duration-300 ease-in-out sm:focus:-translate-x-2/4 focus:scale-150 hover:scale-150"
